...
def design_architecture(challenge: dict, gated_client=None) -> dict:
    ...
    if cont_ctx:
        ...
        _log(
            ...
            f"#{cont_ctx['parent_id']}, architecture frozen, "
            "skipping researcher"
        )
    ...
    if not config_broken:
        # ── Phase 1: researcher ─────────────────────────────────
        # Skipped on continuation rounds — the architecture is fixed, so
        # there's no new design to research; the designer just retunes the
        # recipe. The freed budget rolls into the designer (same deadline).
        if cont_ctx:
