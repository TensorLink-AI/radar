"""Orchestrator for the claude_style_v5 multi-subagent miner.

v5 is a wiring-and-prompts fork of v4 — same analyst → researcher →
designer → recipe-tuner pipeline, same budget split, same tool
surface. The additions live in ``core.recipe_sanity`` (static AST
inspector folded into the post-validate callback), ``prompts.py``
(researcher brief gains a ``divergence_axes`` field, critic system
prompt grows a DIVERGE line), and ``subagents/designer.py`` (callback
wiring). See the package ``__init__.py`` for the v4 → v5 deltas.

The v4 docstring below describes the pipeline; v5 only changes
prompts and callbacks, not the orchestration.
