...
def make_inround_recipe_context(
    arch_source: str,
    recipe_norm: dict | None = None,
) -> dict | None:
    """Synthesize a recipe-only context for an in-round second pass.

    v4 reuses the continuation preamble machinery to run a focused
    training-recipe pass after the designer ships a validated
    architecture. We extract the architecture portion of the just-
    validated code, hand it to the designer as the frozen block, and
    let it spend the remaining budget on the recipe hooks only.

    Differs from ``build_context`` in two ways:
      * No parent_id — there's no warm-start checkpoint to load. The
        sentinel ``_inround_recipe: True`` tells the orchestrator
        ``_cont_fields`` not to emit ``mode="continue"`` so the
        validator scores the submission as a fresh design.
      * ``training_recipe_norm`` is threaded through so the preamble
        can show the designer what the frontier already does and
        what's worth diverging from.

    Returns ``None`` if the architecture can't be extracted (no
    ``build_model``, code didn't parse) — caller should ship the
    original validated candidate untouched.
    """
    if not arch_source:
        return None
    frozen = extract_arch_source(arch_source)
    if not frozen:
        return None
    return {
        "_inround_recipe": True,
        "frozen_arch": frozen,
        "training_recipe_norm": dict(recipe_norm or {}),
    }
