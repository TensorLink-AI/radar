...
def _required_functions(challenge: dict | None) -> dict[str, list[str]]:
    ...
    For the pipeline tasks (``ts_data_pipeline`` / ``synthetic_data_generator``)
    the contract is a ``build_pipeline(...)`` iterator factory; there is no
    ``build_optimizer`` because the harness uses the frozen architecture's
    hooks.
    """
    task_params: dict = {}
    if challenge:
...
def validate_code(code: str, challenge: dict | None = None) -> tuple[bool, list[str]]:
    ...
    # Pipeline tasks ship a data generator, not a model. FLOPs/output-
