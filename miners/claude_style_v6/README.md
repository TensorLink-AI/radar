...
A fork of `miners/claude_style_v4`. v6 inherits v4's entire
architecture-task pipeline (analyst → researcher → designer →
recipe-tuner) and concentrates its additions on the **data-generator
tasks** — `ts_data_pipeline` and `synthetic_data_generator` — where the
designer previously got no signal about whether its generator was any
*good*, only whether the shapes were right.

## What's new vs. `claude_style_v4`

| Concern | `v4` | `v6` |
|---|---|---|
| `synthetic_data_generator` parity | centralised `core.is_pipeline_task` (validation + fallback recognise both pipeline tasks) | inherited unchanged |
...
| Fallback tag | `FALLBACK_VERSION = "v3"` | `FALLBACK_VERSION = "v6"` |

The two v6 mechanisms:

1. **Quality probe.** `core/pipeline_quality.py` computes cheap
...
2. **Design prompt.** `build_pipeline_designer_system_prompt` reframes the
   design space around structural coverage and points the loop at the new
   quality stats.

Everything else is inherited from v4 verbatim.

---

## Inherited from v4 — training-recipe-aware two-pass design
