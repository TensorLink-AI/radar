...
def build_pipeline_designer_system_prompt(
    ...
    """System prompt for the data-pipeline single-designer pass
    (``ts_data_pipeline`` and ``synthetic_data_generator``).

    The miner designs a *data generator / augmentor*, not an architecture.
    The architecture is supplied frozen by the validator so this prompt
    frames the design space, the scoring formula, and the tool surface —
    explicitly suppressing the standard build_model / FLOPs framing the
    designer otherwise uses. v6 rewrites the design space into concrete,
    GIFT-Eval-aware generation recipes with a diversity-first mandate and
    points the loop at the new ``pipeline_smoke_test`` quality stats.
    """
    task = challenge.get("task", {}) or {}
    tp = task.get("task_params", {}) or {}
    param_str = ", ".join(tp.keys()) if tp else "**task_params"
    ...
    parts.append(
        "## Design space — coverage beats cleverness\n\n"
        "GIFT-Eval is ~97 held-out tasks spanning energy, traffic, "
        "weather, finance, sales, web and nature domains at "
        "second→yearly sampling. The fixed model only generalizes to "
        "structure it SAW in training, so the single biggest lever is "
        "**diversity of structure**, not one elegant family. Build a "
        "*mixture* and sample a regime per batch (domain randomization); "
        "per series, compose independent components:\n\n"
        "1. **Trend** — none / linear / piecewise-linear with random "
        "breakpoints / exponential / logistic saturation. Randomize "
        "slope sign and magnitude.\n"
        "2. **Seasonality (multi-scale)** — sum 1-3 cycles at *unrelated* "
        "periods (e.g. daily+weekly, or a random period in "
        "[4, context_len//2]) with random amplitude/phase. Mix in "
        "non-sinusoidal shapes (triangle, square, spike trains) so it "
        "isn't all smooth Fourier modes.\n"
        "3. **Stochastic core** — AR(p)/ARMA, random walk, mean-reverting "
        "(Ornstein-Uhlenbeck), Markov regime-switching (alternate AR "
        "coefficients / variances), change-points (level + variance "
        "shifts).\n"
        "4. **Noise (realistic)** — heteroscedastic (variance scales "
        "with level), heavy-tailed (Student-t), occasional "
        "outliers/spikes, a small fraction of missing/imputed steps. "
        "Pure low-variance Gaussian on a clean signal is the classic "
        "collapse mode.\n"
        "5. **Scale + normalization variety** — randomize per-series "
        "offset and scale across orders of magnitude (the arch's arcsinh "
        "scaler handles it) so the model learns the scale-invariance the "
        "leaderboard demands.\n"
        "6. **Multivariate coupling** (when num_variates>1) — a shared "
        "latent factor plus per-channel idiosyncratic noise, lagged "
        "cross-dependencies, and distinct per-channel seasonality. Do "
        "NOT emit V copies of one series.\n\n"
        "Augmentors over the procedural stream (jitter, magnitude/time "
        "warping, window crop, channel permutation, mixup in time) add "
        "cheap variety on top (Wen 2021 TS-augmentation survey). Real "
        "pretrain shards are NOT reachable here — augment the procedural "
        "stream, don't depend on external data.\n"
        + ("\nA mild **curriculum** (clean early → noisier later) can "
           "still help best-val checkpoint selection, but it is "
           "diagnostics only here — never trade coverage for a smoother "
           "early curve.\n" if is_synth else
           "\n**Hybrid curricula** (clean early → noisier later) "
           "directly help: AULC rewards fast early val-loss drops, which "
           "dominate the trapezoidal area.\n")
    )
    ...
    parts.append(
        ...
        "patch size).\n"
        "- `pipeline_smoke_test(code)` — exec your code, pull a few "
        "batches, verify shapes AND report distribution-quality stats "
        "(mean_channel_std, constant_channel_fraction, "
        "mean_lag1_autocorr, value range, cross_batch_variety) with "
        "degeneracy warnings. Sub-second; use it freely and READ the "
        "stats — clean shapes with a flat/identical/constant "
        "distribution is the #1 way to pass validation and then fail the "
        "GIFT gate.\n"
    ...
    parts.append(
        ...
        + " plane.\n"
        "3. Write `build_pipeline` as a MIXTURE — sample a regime per "
        "batch and compose trend + multi-scale seasonality + stochastic "
        "core + realistic noise. A single clean family is the weak "
        "default.\n"
        "4. `pipeline_smoke_test` until shapes are clean AND the quality "
        "stats look healthy: non-trivial mean_channel_std, low "
        "constant_channel_fraction, mean_lag1_autocorr below ~0.999, "
        "cross_batch_variety > 0. Fix any QUALITY WARNINGS before "
        "moving on — they predict a GIFT-gate collapse.\n"
        "5. `validate_code` → fix any structural error.\n"
        "6. `submit` (stash). Iterate to a candidate that covers "
        "different structure (more regimes, harder noise, coupled "
        "channels) and submit again to compare. The strongest rounds "
        "ship 2 candidates with genuinely different coverage.\n"
    ...
    parts.append(
        ...
        + "- Generators that emit perfectly-clean signals, or one family "
        "repeated. The arch memorizes a single mode; the GIFT-Eval gate "
        "then collapses. `pipeline_smoke_test` flags this as a quality "
        "warning — heed it.\n"
        "- A fixed RNG seed that isn't advanced per batch — every step "
        "sees the same data (the smoke test reports "
        "`batches_identical`).\n"
