...
class TestGatedClient:
    ...
    def test_llm_timeout_longer(self):
        client = GatedClient(["http://proxy.test/"], timeout=15, llm_timeout=120)
        assert client._effective_timeout("http://proxy.test/experiments/recent", None) == 15
        assert client._effective_timeout("http://proxy.test/llm/v1/chat/completions", None) == 120
        assert client._effective_timeout("http://proxy.test/llm/models", None) == 120
    ...
    def test_llm_timeout_default(self):
        ...
        assert client._effective_timeout("http://proxy.test/llm/chat", None) == 120
