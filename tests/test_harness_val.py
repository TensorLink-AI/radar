...
def test_runner_without_compute_val_metrics_works():
    ...
    # Only step + loss + flops — no extra task-specific metrics. The flops
    # key rides along whenever the harness can estimate per-step FLOPs (the
    # 2D TinyModel inputs do support the 6N formula here).
    assert set(entry.keys()) == {"step", "loss", "flops"}
