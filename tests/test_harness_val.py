...
# ── Named val metrics (val_pinball + ncrps + mase) ────────────────

def test_val_history_records_named_metrics_when_runner_provides_them():
    """When the runner exposes compute_transfer_metrics(), each val_history
    entry carries val_pinball + ncrps + mase, plus the legacy ``loss`` alias
    so downstream consumers (dashboard, scoring) keep working."""
    transfer_calls = {"n": 0}

    def loss_fn(preds, targets, _idx):
        return torch.tensor(2.5)

    class _MetricsRunner(_FakeRunner):
        def compute_transfer_metrics(self, model, device, amp_dtype, amp_enabled):
            transfer_calls["n"] += 1
            # Vary so we can prove the values flow through, not be overwritten.
            return {"ncrps": 0.95 + 0.01 * transfer_calls["n"], "mase": 0.85}

    runner = _MetricsRunner(
    ...
    assert transfer_calls["n"] >= 2, (
        "compute_transfer_metrics should be called at every val cadence tick"
    )
    assert out["val_history"], "val should have fired"
    for entry in out["val_history"]:
        assert "val_pinball" in entry, entry
        assert "ncrps" in entry, entry
        assert "mase" in entry, entry
        # ``loss`` stays as the val_pinball alias so legacy chart code still
        # has something to plot without a backfill.
        assert entry["loss"] == pytest.approx(entry["val_pinball"])
        assert entry["val_pinball"] == pytest.approx(2.5)
        assert entry["mase"] == pytest.approx(0.85)


def test_compute_transfer_metrics_failure_does_not_kill_val():
    """If compute_transfer_metrics() raises, val still records val_pinball —
    transfer-metric failures must never cascade into training termination.
    """
    class _BrokenRunner(_FakeRunner):
        def compute_transfer_metrics(self, model, device, amp_dtype, amp_enabled):
            raise RuntimeError("simulated transfer-metric failure")

    runner = _BrokenRunner(
        train_batches=[_make_batch() for _ in range(20)],
        val_batches=[_make_batch() for _ in range(2)],
        default_loss_fn=lambda p, t, _i: torch.tensor(1.5),
    )
    sub = _make_submission(training_config={
        "batch_size": 2, "log_every_n_steps": 1000,
        "val_schedule": "fixed", "val_base_step": 1,
    ...
    assert out["val_history"], "val_pinball should still be recorded"
    for entry in out["val_history"]:
        assert "val_pinball" in entry
        assert "ncrps" not in entry
        assert "mase" not in entry
        assert entry["loss"] == pytest.approx(1.5)


def test_compute_transfer_metrics_filters_non_finite_and_partial():
    """Non-finite or non-numeric metric values are dropped; partial returns
    (only ncrps, only mase, etc.) are honored as-given.
    """
    class _PartialRunner(_FakeRunner):
        def compute_transfer_metrics(self, model, device, amp_dtype, amp_enabled):
            return {"ncrps": float("nan"), "mase": 0.7, "extra": "ignored"}
    ...
    for entry in out["val_history"]:
        assert "val_pinball" in entry
        assert "mase" in entry and entry["mase"] == pytest.approx(0.7)
        # NaN ncrps and the "extra" key are both dropped.
        assert "ncrps" not in entry
