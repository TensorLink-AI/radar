...
class TSForecastingRunner:
    ...
    def compute_transfer_metrics(
        ...
        """Run a small GIFT-Eval slice and return ``{ncrps, mase}``.

        These are the transfer / point-quality signals — geo-mean ratios
        against seasonal-naive — that complement the IID ``val_pinball``
        signal. They're meaningful but expensive, so by default we run only
        a handful of representative datasets per cadence tick.

        Honored env vars:
          RADAR_VAL_TRANSFER_METRICS  ``0`` disables; default enabled.
          RADAR_VAL_TRANSFER_DATASETS comma-separated GIFT-Eval names; if set
                                      we use exactly those, else the first
                                      RADAR_VAL_TRANSFER_DATASET_LIMIT (default
                                      3) entries from SHORT_DATASETS.
          RADAR_VAL_TRANSFER_BATCH_SIZE override for the per-task batch size.
        """
        import os
        if os.environ.get("RADAR_VAL_TRANSFER_METRICS", "1").lower() in ("0", "false", "no", "off"):
            return None

        try:
            from prepare import validate
        except Exception as e:
            logger.warning("compute_transfer_metrics: prepare.validate import failed: %s", e)
            return None

        names: list[str] | None = None
        env_names = os.environ.get("RADAR_VAL_TRANSFER_DATASETS", "").strip()
        if env_names:
            names = [n.strip() for n in env_names.split(",") if n.strip()]
        else:
            try:
                from shared.gift_eval import SHORT_DATASETS
                limit = int(os.environ.get("RADAR_VAL_TRANSFER_DATASET_LIMIT", "3"))
                limit = max(1, min(limit, len(SHORT_DATASETS)))
                names = list(SHORT_DATASETS[:limit])
            except Exception as e:
                logger.warning("compute_transfer_metrics: SHORT_DATASETS unavailable: %s", e)

        try:
            batch_size = int(os.environ.get("RADAR_VAL_TRANSFER_BATCH_SIZE", "32"))
        ...
        try:
            ...
            try:
                with torch.no_grad():
                    metrics = validate(
                        model, batch_size=batch_size, dataset_names=names,
                    )
            finally:
                if was_training:
                    model.train()
        except Exception as e:
            logger.warning("compute_transfer_metrics: validate() failed: %s", e)
            return None
