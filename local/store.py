...
class LocalStore:
    ...
    def _ensure_columns(self) -> None:
        """Additively migrate older DBs: add columns absent in old schemas.

        SQLite tolerates ``ALTER TABLE ADD COLUMN`` with a default, so old
        experiment rows read back with sensible defaults (``val_curve``
        empty, ``n_rounds=1``, ``mode='new'``) and nothing breaks.
        """
        cols = {
            r["name"]
    ...
    def add_experiment(self, *, round_id: int, miner_id: str, name: str,
                       ...
                       task: str = "", val_curve: Optional[list] = None,
        ...
        with self._tx() as c:
            cur = c.execute(
                ...
                " generation, prompt_id, task, checkpoint_ref, n_rounds, "
